...
def clean_invalid_external_ids(cursor, table_name, invalid_ids):
    ...
    if not invalid_ids:
        print(f"No invalid external IDs to clean for {table_name}")
        return
...
def process_table(cursor, table_name):
    ...
    cursor.execute(f"SELECT id, external_id, title FROM {table_name} where created >= '2025-07-01' AND ror_id IS NULL AND ror_name IS NULL;")
...
def process_by_titles(cursor, table_name):
    ...
    cursor.execute(f"SELECT id, external_id, title FROM {table_name} where created >= '2025-07-01' AND external_id IS NOT NULL AND ror_id IS NULL;")
    rows = cursor.fetchall()
